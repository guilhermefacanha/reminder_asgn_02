...
/**
 * 
 * @author Reminder Group Class is responsible for managing the notification Data
...
public class NotificationDAO implements Serializable
{
	...
	/**
	 * Method which retrieves all the notifications of a user and returns a list of them
	 * 
	 * @param user
	 *            - provides the user which the program will load the notifications from
	 * @return - returns a list of notifications objects from that user
	 */
	@SuppressWarnings("unchecked")
	public List<Notification> getAllNotifications(User user) {
		EntityManager manager = Resources.getEntityManager();
		List<Notification> notifications = manager.createQuery("SELECT x FROM Notification x WHERE x.event.course.user.id = :id")
	...
	public void deleteNotification(Long notificationId) {
		...
		try { 
			...
			manager.createQuery("DELETE FROM Notification x WHERE x.id = :id").setParameter("id", notificationId).executeUpdate();
